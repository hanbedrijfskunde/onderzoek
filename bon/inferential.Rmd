...
url <- "https://docs.google.com/spreadsheets/d/1F_fOXfNjLzx_T_cDPEyMNMV60CgmxlBpM_XYr0D7sKc/edit?usp=sharing"
...
pltPrijs <- ggplot(myDF) +
  geom_point(aes(x = Zoektijd, y = `Wat is de prijs van het product die u heeft gevonden?`, color = `Wat is uw geslacht`))
ggplotly(pltPrijs, width = 800)

pltPrijs <- ggplot(myDF) +
  geom_jitter(aes(x = Zoektijd, y = `Wat is de prijs van het product die u heeft gevonden?`, color = `Wat is uw geslacht`), width = 0.20)
ggplotly(pltPrijs, width = 800)
```
