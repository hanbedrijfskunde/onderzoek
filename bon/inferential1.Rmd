...
myDF <- gsheet2tbl(url)[-3] %>% filter(., `Bentustudent` == "Ja")
str(myDF)
```
...
pltZoek <- ggplot(myDF) +
  geom_bar(aes(Zoektijd, fill = `Watisuwgeslacht`))
ggplotly(pltZoek, width = 800)

# Zoektijd versus prijs

pltPrijs <- ggplot(myDF) +
  geom_point(aes(x = Zoektijd, y = `Watisdeprijsvanhetproductdieuheeftgevonden`, color = `Watisuwgeslacht`))
ggplotly(pltPrijs, width = 800)

pltPrijs <- ggplot(myDF) +
  geom_jitter(aes(x = Zoektijd, y = `Watisdeprijsvanhetproductdieuheeftgevonden`, color = `Watisuwgeslacht`), width = 0.20)
ggplotly(pltPrijs, width = 800)
```
